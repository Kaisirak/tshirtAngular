...
(function(){
	...
	app.directive('ngGridItems', function($window, $timeout) {
		return {
			...
			template: '<ul class="cs-grid-items"><li ng-repeat="item in displayList" style="height: {{itemHeight}}px; width: {{itemWidth}}px; background-color: {{item.color}}; -webkit-transform: translate3D({{item.positionX}}px, {{item.positionY}}px, 0px) scale3D({{item.active?1:0.001}},{{item.active?1:0.001}},{{item.active?1:0.001}}); transform: translate3D({{item.positionX}}px, {{item.positionY}}px, 0px) scale3D({{item.active?1:0.001}},{{item.active?1:0.001}},{{item.active?1:0.001}});opacity: {{item.active?1:0}}"> <div class="" style="height:300px;">  <div class="caption"><center><p style="margin: 10px;">{{ item.txt }}</p></center></div> <img class="img-responsive" ng-src="{{ item.img }}" alt="Slide11" height="200"><div class="caption"><center><p style="margin: 10px;"><a href="{{ item.url }}" class="btn btn-success" style="margin: 2px;" role="button">View</a><a href="{{ item.url }}" class="btn btn-primary ng-binding" style="margin: 2px;" role="button">{{ item.price | currency }}</a></p></center></div></div></li></ul>',
				...
				function(newVal){
					...
					$scope.updateView = function()
					{
						for (var i = iAct = 0; i < $scope.displayList.length; i++)
						{
							...
							$scope.displayList[i].positionX = ((iAct * $scope.itemWidth) % $scope.divWidth) + posX * $scope.itemRightMargin + $scope.divX + (($scope.realWidth - $scope.divWidth) / 2);
	...
	app.controller('MainController', ['$http', '$scope', '$location', function($http,$scope,$location) {
		...
		this.addProductToCart = function(obj, variant, varprice) {
			console.log(obj);
			var cartObj = {size: variant.size, color: obj.product_variants[variant.colorid].color_name, prod: obj.name, price: varprice};
			this.cart.push(cartObj);
			$('#cartModal').modal('show');
		};
