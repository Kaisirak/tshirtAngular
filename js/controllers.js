...
(function(){
	...
	app.directive('ngGridItems', function($window, $timeout) {
		return {
			...
			template: '<ul class="cs-grid-items"><li ng-repeat="item in displayList" style="height: {{itemHeight}}px; width: {{itemWidth}}px; background-color: {{item.color}}; -webkit-transform: translate3D({{item.positionX}}px, {{item.positionY}}px, 0px) scale3D({{item.active?1:0.001}},{{item.active?1:0.001}},{{item.active?1:0.001}}); transform: translate3D({{item.positionX}}px, {{item.positionY}}px, 0px) scale3D({{item.active?1:0.001}},{{item.active?1:0.001}},{{item.active?1:0.001}});opacity: {{item.active?1:0}}"> <div class="thumbnail" style="height:300px;">  <div class="caption"><center><p>{{ item.txt }}</p></center></div> <img ng-src="{{ item.img }}" alt="Slide11" height="200"><div class="caption"><center><p style="margin-bottom: 5px;"><a href="{{ item.url }}" class="btn btn-success" role="button">View</a><a href="{{ item.url }}" class="btn btn-primary ng-binding" role="button">{{ item.price | currency }}</a></p></center></div></div></li></ul>',
